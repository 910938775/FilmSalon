...
switch 条件语句，不是比较 Boolean 值，而是 值 的各种可能性，其中就包括 undefined = false。

**10.经验总结：**

Vue 父子组件之间传递方法时，如果需要 **传递参数**，那么在 标签内部 填写方法时，不要加上 括号，否则方法会 从该处调用参数（空）。

Vue 模板标签内部，使用 **事件函数**时，不要使用 this 对象，webpak 编译时，会自动加上 vue this 对象，否则会报错 不存在。

**11.经验总结：**

在 webpack 编译的 css 中，任何 js 创建的 字符串标签 都无法使用该 css 样式（V-html, creatElement 等），必须使用 V-for 来创建，以保证循环创建的 **根标签** 存在，并经过 webpack 编译。

- **手机端重置**

**1.经验总结：**

网页内部 二次请求地址 规范：**域名 + 端口号 + 文件绝对路径**

目的：解决跨域访问中，文件绝对路径 会自动添加当前网页 域名端口 为 路径前缀，导致访问错误。

**2.经验总结：**

伪选择器：``nth-child(even|odd) nth-type-of(even|odd)``
不能对父元素使用，必须对 子元素集合 使用。
( 例如：ul > li:nth-child(even|odd) )

**3.经验总结：**

'uni-app' 中 @xxx 绑定函数，不能使用 this 关键字，会导致找不到。

**4.经验总结：**

``vue $refs`` 获取的组件，若是 html 基本标签，则可以直接使用 DOM 对象;若是 vue 组件，则是个 vue 对象，
不可以直接使用 DOM 对象，可以使用 $el $event 获取对象。

**5.经验总结：**

'uni-app' 中 ``$this $event $el`` 都对进行了重新定义？

**6.经验总结：**

'uni-app' 中 **表单组件** 对原始组件进行了改造，无法在 css 中进行 **表单伪选择器**。

**7.经验总结：**

``a`` 标签在添加 **隐藏 visibility** 属性后，无法触发 ``:active`` 等方法。

表单组件 在隐藏后，仍然可以触发相关方法 ``:focus :checked`` 等，因此可以利用这种特性来代替 js 点击事件添加样式。

**8.经验总结：**

父子组件之间，特别是 **开源式组件**，要做到 **解耦合**，不要让外部的 **属性 方法** 决定内部的属性方法的最终应用效果。

**9.经验总结：**

``connect-history-api-fallback`` 跨域地址也会进入处理？

**10.经验总结：**

'uni-app' 中，组件 prop 属性中，不能使用 ``~@/xxx/`` 绝对路径，这样会导致路径错误，只能使用 ``/xxx/`` 方式。

**11.经验总结：**

'uni-app vue' 中，不能直接对 Data 数据进行循环操作，这样会导致 第一操作就让数据生效，而后的所有操作都没有用，只能把数据
封装完毕后，才能赋值给 Data 数据。
...
``uni.navigateTo()`` 页面跳转 API，url 路径 必须遵守 ``/xxx/xxx/xxx`` 的格式。
回调函数中，也存在 this 指向性问题。

**14.经验总结：**

css 中 子元素 设置 ``overflow`` 时，可以覆盖到 父元素 的 border 区域以内。

**15.经验总结：**

uni-app 中，``ref`` 属性获取时，如果是 ``v-for`` 创建的节点，则获取到的是个 **伪数组**，正常创建的则是 **对象**。

**16.经验总结：**

uni-app 中，``input`` 密码框在使用 ``type: password`` 属性时，如果使用 ``v-model`` 则会使用原始表单样式，只能使用
``passsword: true`` 的属性。
...
正则表达式中，``[]``中不能使用 ``|`` ``()`` 符号，因为默认会把 [] 中所有对象分解成成 **单一对象**。
...
**同步函数** 内部可以 **永久赋值 全局变量**，不会随着函数被 **销毁** 而恢复原来的值。

**异步函数** 内部之所以不可以 **永久赋值 全局变量**，不是因为函数被销毁的问题，而是函数内部 **执行顺序** 的问题，而是因为
**异步数据结果** 晚于 **变量赋值成功**。
...
console.log(a)  // 4. 调用变量 ---> a = 1

// 原因：变量提升
// ===> 相当于下面的执行过程：
function b(c) {  // 被提升执行
  var a = c  // 作用域中没有，自己创建新的变量
};
b(2);
var a = 1;
console.log(a)
...
var a = 1;
function b(c) {
  var a = 3;  // 就近被变量提升：使之成为了 函数作用域的变量
  a = c
};
b(2);
console.log(a)
...
uni-app 中对 **事件函数参数 e||event** 进行了再封装，无法直接获取节点了，但可以利用 原生的 **window.event||event** 对象。

**21.经验总结：**

js 中，scrollWidth = offsetLeft + clientWidth。

**22.经验总结：**

js 中，``margin pading offset scroll`` 等 距离属性，都是相对于 **父元素 content** 区域。

**23.经验总结：**

css 动态渲染总结：

``attr()``：该属性可以动态的把标签上的 data 属性，使用在 css 上。

**24.经验总结：**

uni-app 生命周期中，**全局组件** 的 **页面生命周期** 不起作用，而 vue 的生命周期起作用。

原因：没有在 page.json 中注册页面导致？或者全局加载先于单个页面加载？

**25.经验总结：**

css 中，``display: flex`` 中，不支持设置 ``text-overflow`` 属性。

原因：flex 布局，有自己的溢出处理机制？

**26.经验总结：**

``width: max-content`` 可以自动根据 **内容** 来处理 **显示** 问题，当没有内容时就不会显示。

**27.经验总结：**

async/await 函数 对于 **独立的生命周期函数** 来说，没有效果，它们依旧会按照本来的 **顺序执行**，只有在一个 **封闭函数内部** 才有效果。

**28.经验总结：**

表单组件中 **select** 下拉组件，下拉框 **option** 没法通过 **css** 与父容器 select 动态的 保持**相等大小**。

原因：option 是通过 js 动态创建的？

**29.经验总结：**

``css position:absolute` 中，``top/bottom/left/right:100% != top/bottom/left/right:-100%``，其原因在于

这些定位只是相对于 **对应的边** 而言，要想达到 子元素 与 父元素 处于 **外部紧挨（下拉菜单样式）** 着的布局，

只能使用 100% **正向移动**，而 -100% **反向移动**并不能使之 父子外部紧挨，要么 父子远离，要么 父子嵌套。

**30.经验总结：**

``width/height/pading/margin/top/bottom/left/right`` 等属性值 %，都是相对于 ``pading + content`` 区域而言。

**31.经验总结：**

vue 中，通过 表达式 绑定的标签属性值，无法动态更新，必须通过 属性绑定 的方式实现。
...
``css transform`` 无法同时绑定 ``rotate translate`` ？
