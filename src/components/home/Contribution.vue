    ...
    <div class="Contribution-root">
        ...
        <div class="Contribution">
            ...
            <div class="ContributionContent">
                <!-- 用户贡献榜 内容 二级容器 -->
                <div class="ContentContainer" v-for="(item, i) in UsersContribution" :key="i">

                    <!-- 用户贡献榜 内容 左边区域 容器 -->
                    <div class="LeftContentContainer">
                        <span class="ContentRanking">{{ item.Ranking }}</span>
                        <a class="ContentUsr" href="">{{ item.NickName }}</a>
                    </div>

                    <!-- 用户贡献榜 内容 右边区域 容器 -->
                    <div class="RightContentContainer">
                        <span>{{ item.ContributionValue }}</span>
                    </div>
...
export default {
    ...
    data() {
        return {
            ...
            UsersContribution: [],
            // 后台数据接口 绝对路径
            UsersContributionPath: '/index/contribution/'
        }
    },
    // 方法 接口
    methods: {
        ...
        async getContribution() {
            // 封装 请求数据 对象
            let getData = {
                url: this.UsersContributionPath,
                tool: this
            };
            // 实例化请求对象
            ...
            (result.status === 0) && (
                // 赋值 数据
                this.UsersContribution = result.content
            );
            // 数据 获取失败
            (result.status === 1) && (
                Toast({
...
// 用户贡献榜 内容 左边容器 用户名 鼠标交互设计
// 清除 a链接 默认字体颜色 覆盖问题
.ContentUsr {
    text-decoration: none; color: #696969
}
.ContentUsr:hover, .ContentUsr:focus {
...
@media(max-width: 768px) {
    // 重定义 用户贡献榜 根节点样式
    .Contribution-root {
        width: 95%; left: 2.5%; height: 210px; top: 5px
    }
}
...
@media(max-width: 630px) {
    ...
    // 重定义 用户贡献榜 左边容器 排名 基本样式
    .TitleRanking {
        width: 25%
    }

    // 重定义 用户贡献榜 左边容器 用户名 基本样式
    .TitleUsr {
        width: 70%
    }
