...
<template>
    ...
    <div class="Course-root" ref="Course">
        ...
        <!-- 其他视频类课程 热度值榜 组件 -->
        <!-- 其他课程组件 外部一级容器 -->
        <div class="Course" v-for="(VIndex, i) in OtherCourseVValue" :key="i" v-show="Type">
            <!-- 其他课程组件 外部二级容器 -->
        ...
        <!-- 其他图文类课程 热度值榜 组件 -->
        <!-- 其他课程组件 外部一级容器 -->
        <div class="Course" v-for="(ITIndex, i) in OtherCourseITValue" :key="'IT-' + i" v-show="!Type">
            <!-- 其他课程组件 外部二级容器 -->
            <div class="CourseExternal">
                <!-- 其他课程组件 外部三级容器 -->
                <div class="CourseContainer">
...
export default {
    ...
    data() {
        return {
            ...
            OtherCourseITValue: [],
            // 后台数据接口 绝对路径
            OtherCoursePath: '/index/otherCourse/',
            // 其他课程 视频 图文 类型判断
            Type: true,
            // 其他课程 分类内容容器 标签动画 加速度
            Accelerated: 5,
            // 其他课程 分类内容容器 标签动画 时间
            Time: 1000,
            // 视频其他课程 分类内容容器 标签动画 速度累加器
            VSpeed: 0,
            // 视频其他课程 分类内容容器 标签动画 定时器
            VTimer: '',
            // 图文其他课程 分类内容容器 标签动画 速度累加器
            ITSpeed: 0,
            // 图文其他课程 分类内容容器 标签动画 定时器
            ITTimer: '',
            // 其他课程 分类内容容器 标签动画 滚动速度
            RollSpeed: 1000
        }
    },
    // 方法 接口
    methods: {
        ...
        Parent() {
            // 获取 模板根节点 父节点
            let node = this.$refs.Course.parentNode.classList.contains('right-down-container');
            // 如果 父节点 是图文区 则 隐藏 视频节点
            if (node) {
                this.Type = false
            }
        },
        // 获取 后台发过来的 其他课程 热度值数据
        async getFilmCourse() {
            // 封装 请求数据 对象
            let getData = {
                url: this.OtherCoursePath,
                tool: this
            };
            // 实例化请求对象
            ...
            (result.status === 0) && (
                // 获取 视频课程 数据部分
                this.OtherCourseVValue = result.content.video,
                // 获取 图文课程 数据部分
                this.OtherCourseITValue = result.content.imageText
            );
            // 数据 获取失败
            (result.status === 1) && (
                Toast({
...
// 其他热度值榜区域 名称内容容器 基本样式
.CourseName-Content {
    display: flex; position: relative; width: 78%; height: 100%;
    color: rgba($color: #D2691E, $alpha: 1); overflow-y: scroll; text-decoration: none;
    align-items: center
}
// 隐藏 overflow 的滚动条
...
// 其他热度值榜区域 名称内容容器 鼠标交互样式
.CourseName-Content:hover {
    font-weight: bold
}


// 其他热度值榜区域 分类外部容器 基本样式
.CourseTag {
...
// 其他热度值榜区域 分类内容容器 鼠标交互样式
.CourseTag-Content > a:hover{
    font-weight: bold
}


/* 首页 其他热度值榜区域 样式 响应式布局 */
// 手机端 显示效果
