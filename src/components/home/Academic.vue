    ...
    <div class="Academic-root">
        ...
        <div class="Academic">
            ...
            <div class="AcademicContent">
                <!-- 学术热度值榜 内容 二级容器 -->
                <div class="ContentContainer" v-for="(item, i) in AcademicValue" :key="i">
...
export default {
    ...
    methods: {
        ...
        async getAcademic() {
            ...
            (result.status === 0) && (
                // 赋值 数据
                this.AcademicValue = result.content
            );
            // 数据 获取失败
            (result.status === 1) && (
                Toast({
...
/* 首页 学术热度值榜区域 样式 响应式布局 */
// 平板端 显示效果
@media(max-width: 960px) {
    // 重定义 学术热度值榜 标题容器 基本样式
    .AcademicTitle {
        font-size: 13px
    }

    // 重定义 学术热度值榜 内容一级容器 基本样式
    .AcademicContent {
        font-size: 11px
    }

    // 重定义 学术热度值榜 左边容器 基本样式
    .LeftTitleContainer, .LeftContentContainer {
        width: 70%
    }

    // 重定义 学术热度值榜 右边容器 基本样式
    .RightTitleContainer, .RightContentContainer {
        width: 25%
    }
}
...
@media(max-width: 768px) {
    // 重定义 学术热度值榜 根节点样式
    .Academic-root {
        width: 95%; left: 2.5%; height: 210px; top: 10px
    }
}
...
@media(max-width: 630px) {
    ...
    // 重定义 学术热度值榜 左边容器 排名 基本样式
    .TitleRanking {
        width: 25%
    }

    // 重定义 学术热度值榜 左边容器 学术名 基本样式
    .TitleName {
        width: 70%
    }
